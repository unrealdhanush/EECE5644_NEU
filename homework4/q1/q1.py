...
def plot_data(TrainingData_labels, TrainingData_features, TestingData_labels, TestingData_features): 
    plt.subplot(1,2,1) 
    plt.plot(TrainingData_features[np.array((TrainingData_labels==0).nonzero())][0,:,0],TrainingData_features[np.array((TrainingData_labels==0).nonzero())][0,:,1],'b.') 
    ...
    plt.subplot(1,2,2)
    plt.plot(TestingData_features[np.array((TestingData_labels==0).nonzero())][0,:,0], TestingData_features[np.array((TestingData_labels==0).nonzero())][0,:,1], 'b.')
    plt.plot(TestingData_features[np.array((TestingData_labels==1).nonzero())][0,:,0], TestingData_features[np.array((TestingData_labels==1).nonzero())][0,:,1], 'm.')
    plt.show()
...
def train_SVM_hyperparams(TrainingData_labels, TrainingData_features):
    ...
    for (i, hyperparams) in enumerate(np.reshape(np.transpose(hyperparam_candidates), (-1, 2))): skf = StratifiedKFold(n_splits=K, shuffle=False)
    total_accuracy = 0
    for(k, (train, test)) in enumerate(skf.split(TrainingData_features, TrainingData_labels)): (_, accuracy) = SVM_accuracy(hyperparams, TrainingData_features[train],
...
(SVM_model, SVM_performance) = SVM_accuracy(SVM_hyperparams, TRData_features, TRData_labels, TestData_features, TestData_labels)
(MLP_model, MLP_performance) = max(map(lambda _: MLP_accuracy(MLP_hyperparams, TRData_features, TRData_labels, TestData_features, TestData_labels), range(5)), key=lambda r: r[1])
